...
def test_option_3_True(controller):
    """ test to go through option 3 and add a new employee to the database """
    with mock.patch.object(builtins, 'input', side_effect=[3, "mo","mohseni", "15151512 2020","778452112", "specialist", 6, 6]):
        check = controller.run()
        assert check == False


def test_option_3_existing_id(controller):
    """ test to go through option 3 and try to add an employee ID that already exists """
    with mock.patch.object(builtins, 'input', side_effect=[3, "mo","mohseni", "15151512 2020", "999-999-9999", "specialist", 6]):
        check = controller.run()
        assert check == False


def test_option_3_no_fname(controller):
    """ test to go through option 3 and try to add an employee with no first name """
    with mock.patch.object(builtins, 'input', side_effect=[3, "", "mohseni", "15151512 1999", "999-999-9999", "specialist", 6]):
        check = controller.run()
        assert check == False


def test_option_3_no_lname(controller):
    """ test to go through option 3 and try to add an employee with no last name """
    with mock.patch.object(builtins, 'input', side_effect=[3, "mo", "", "15151512 1999", "999-999-9999", "specialist", 6]):
        check = controller.run()
        assert check == False


def test_option_3_no_position(controller):
    """ test to go through option 3 and try to add an employee with no position """
    with mock.patch.object(builtins, 'input', side_effect=[3, "mo","mohseni", "15151512 1999", "999-999-9999", "", 6]):
        check = controller.run()
        assert check == False


def test_option_3_no_ID(controller):
    """ test to go through option 3 and try to add an employee with no ID """
    with mock.patch.object(builtins, 'input', side_effect=[3, "mo","mohseni", "", "999-999-9999", "specialist", 6]):
        check = controller.run()
        assert check == False
